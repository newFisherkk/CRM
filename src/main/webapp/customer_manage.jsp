  ...
  <body>
	...
	<div id="win" class="easyui-window" title="客户数据编辑" style="width:500px;height:300px"   
	    ...
	    <form id="editForm" method="post">
	    	<%--提供id隐藏域 hidden class="easyui-validatebox" 表示必须填写--%>
  		...
  		$(function(){
  			...
  			$("#saveBtn").click(function(){
  				$("#editForm").form("submit",{
  					...
  					//success:服务器执行完毕回调函数
  					success:function(data){ //data: 服务器返回的数据，JSON类型
  						//Controller返回的数据格式：  
  						//成功：{"success":true} 失败:{"success":false,"msg":"错误信息"}
  						
/*   		     change the JSON string to javascript object
  很关键，这里Control返回来的是Map JSON字符串格式  而SSM-CRUD项目里返回的是Msg对象，
  所以可以直接.success而不用转换
加上圆括号的目的是迫使eval函数在处理JavaScript代码的时候强制将括号内的表达式(expression)转化为对象，
而不是作为语句(statement)来执行。举一个例子，例如对象字面量{}，如若不加外层的括号，
...
console.log(eval("({})");    object[Object] */
  			...
  			$("#editBtn").click(function(){
  	         ...
  	         //表单回显,虽然表是POST，但是要通过URL把id传过去(没有清空表单数据)  rows[0]表示一个DOM对象
  			...
  			$("#deleteBtn").click(function(){
  				...
  				//格式： id=1&id=2&id=3  当用户点击确认按钮时传递一个 true 参数给函数，否则给它传递一个 false 参数
...
//传递到后台??------------------这里的data为何是个JSON对象:后面写了"json" - 以 JSON 运行响应，并以 JavaScript 对象返回-----------------
